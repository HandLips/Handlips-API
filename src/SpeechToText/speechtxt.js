...
const storage = new Storage({
  keyFilename: path.join(__dirname, "speech-to-text.json"),
});
...
// Model Speechtotext
const Speechtotext = sequelize.define(
  "Speechtotext",
  {
    id: {
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "Format file tidak didukung. Hanya WAV, OGG, MP3 diperbolehkan."
        )
      );
    }
  },
...
async function convertAudioToWavBuffer(inputBuffer) {
  try {
    ...
    execSync(
      `${ffmpeg} -i ${tempWavFile} -acodec pcm_s16le -ar 16000 ${outputWavFile}`
    );
...
async function convertToMonoBuffer(inputBuffer) {
  ...
  const monoChannel = decoded.channelData[0].map(
    (_, i) => (decoded.channelData[0][i] + decoded.channelData[1][i]) / 2
  );
...
app.post("/speechtotext", upload.single("audio"), async (req, res) => {
  try {
    ...
    if (!email || !title || !req.file) {
      return res
        .status(400)
        .json({ error: "Email, title, and audio file are required" });
    }
    ...
    const transcriptionUrl = await saveTranscriptionToCloud(
      transcription,
      title
    );
...
app.get("/speechtotext/:email", async (req, res) => {
  ...
  try {
    ...
    if (transcriptions.length === 0) {
      return res
        .status(404)
        .json({ message: "No transcriptions found for this email" });
    }
